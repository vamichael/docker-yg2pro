...
def setup_platform(hass, config: ConfigType,
    ...
    """Set up the isy994mrf fan platform."""
    devices = []
...
class ISYFanDevice(ISYDevice, FanEntity):
    """Representation of an isy994mrf fan device."""
    ...
    def set_speed(self, speed: str) -> None:
        """Send the set speed command to the isy994mrf fan device."""
        self._node.on(val=STATE_TO_VALUE.get(speed, 255))

    def turn_on(self, speed: str = None, **kwargs) -> None:
        """Send the turn on command to the isy994mrf fan device."""
        self.set_speed(speed)

    def turn_off(self, **kwargs) -> None:
        """Send the turn off command to the isy994mrf fan device."""
        self._node.off()
...
class ISYFanProgram(ISYFanDevice):
    """Representation of an isy994mrf fan program."""

    def __init__(self, name: str, node, actions) -> None:
        """Initialize the isy994mrf fan program."""
        super().__init__(node)
        self._name = name
        self._actions = actions

    def turn_off(self, **kwargs) -> None:
        """Send the turn on command to isy994mrf fan program."""
        if not self._actions.runThen():
            _LOGGER.error("Unable to turn off the fan")

    def turn_on(self, speed: str = None, **kwargs) -> None:
        """Send the turn off command to isy994mrf fan program."""
