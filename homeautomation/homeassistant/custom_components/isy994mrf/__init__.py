...
isy994mrf_NODES = "isy994mrf_nodes"
isy994mrf_WEATHER = "isy994mrf_weather"
isy994mrf_PROGRAMS = "isy994mrf_programs"
...
def _check_for_node_def(hass: HomeAssistant, node,
    ...
    for domain in domains:
        if node_def_id in NODE_FILTERS[domain]['node_def_id']:
            hass.data[isy994mrf_NODES][domain].append(node)
            return True
    ...
    for domain in domains:
        if any([device_type.startswith(t) for t in
            ...
            if domain == 'fan' and int(node.nid[-1]) == 1:
                hass.data[isy994mrf_NODES]['light'].append(node)
                return True
...
def _check_for_uom_id(hass: HomeAssistant, node,
    ...
    if uom_list:
        if node_uom.intersection(uom_list):
            hass.data[isy994mrf_NODES][single_domain].append(node)
            return True
    else:
        domains = SUPPORTED_DOMAINS if not single_domain else [single_domain]
        for domain in domains:
...
def _categorize_nodes(hass: HomeAssistant, nodes, ignore_identifier: str,
    ...
    for (path, node) in nodes:
        ...
        if isinstance(node, Group):
            hass.data[isy994mrf_NODES][SCENE_DOMAIN].append(node)
            continue
        ...
        if sensor_identifier in path or sensor_identifier in node.name:
            ...
            if _is_sensor_a_binary_sensor(hass, node):
                ...
                hass.data[isy994mrf_NODES]['sensor'].append(node)
                continue
...
def _categorize_programs(hass: HomeAssistant, programs: dict) -> None:
    """Categorize the isy994mrf programs."""
    for domain in SUPPORTED_PROGRAM_DOMAINS:
        try:
            ...
            for dtype, _, node_id in folder.children:
                ...
                hass.data[isy994mrf_PROGRAMS][domain].append(entity)


def _categorize_weather(hass: HomeAssistant, climate) -> None:
    """Categorize the isy994mrf weather data."""
    ...
    hass.data[isy994mrf_WEATHER].extend(weather_nodes)


def setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up the ISY 994 platform."""
    hass.data[isy994mrf_NODES] = {}
    for domain in SUPPORTED_DOMAINS:
        hass.data[isy994mrf_NODES][domain] = []

    hass.data[isy994mrf_WEATHER] = []

    hass.data[isy994mrf_PROGRAMS] = {}
    for domain in SUPPORTED_DOMAINS:
        hass.data[isy994mrf_PROGRAMS][domain] = []
    ...
    if host.scheme == 'http':
        ...
        _LOGGER.error("isy994mrf host value in configuration is invalid")
        return False
...
class ISYDevice(Entity):
    """Representation of an isy994mrf device."""
    ...
    def on_update(self, event: object) -> None:
        """Handle the update event from the isy994mrf Node."""
        self.schedule_update_ha_state()

    def on_control(self, event: object) -> None:
        """Handle a control event from the isy994mrf Node."""
        self.hass.bus.fire('isy994mrf_control', {
