...
def setup_platform(hass, config: ConfigType,
    ...
    """Set up the isy994mrf lock platform."""
    devices = []
...
class ISYLockDevice(ISYDevice, LockDevice):
    """Representation of an isy994mrf lock device."""

    def __init__(self, node) -> None:
        """Initialize the isy994mrf lock device."""
        super().__init__(node)
    ...
    def lock(self, **kwargs) -> None:
        """Send the lock command to the isy994mrf device."""
    ...
    def unlock(self, **kwargs) -> None:
        """Send the unlock command to the isy994mrf device."""
        # Hack until PyISY is updated
        req_url = self._conn.compileURL(['nodes', self.unique_id, 'cmd',
                                         'SECMD', '0'])
        response = self._conn.request(req_url)
