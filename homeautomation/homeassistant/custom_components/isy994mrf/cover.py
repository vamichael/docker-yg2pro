...
def setup_platform(hass, config: ConfigType,
    ...
    """Set up the isy994mrf cover platform."""
    devices = []
...
class ISYCoverDevice(ISYDevice, CoverDevice):
    """Representation of an isy994mrf cover device."""
    ...
    def is_closed(self) -> bool:
        """Get whether the isy994mrf cover device is closed."""
        return self.state == STATE_CLOSED

    @property
    def state(self) -> str:
        """Get the state of the isy994mrf cover device."""
        if self.is_unknown():
            return None
        return VALUE_TO_STATE.get(self.value, STATE_OPEN)

    def open_cover(self, **kwargs) -> None:
        """Send the open cover command to the isy994mrf cover device."""
        if not self._node.on(val=100):
            _LOGGER.error("Unable to open the cover")

    def close_cover(self, **kwargs) -> None:
        """Send the close cover command to the isy994mrf cover device."""
        if not self._node.off():
            _LOGGER.error("Unable to close the cover")


class ISYCoverProgram(ISYCoverDevice):
    """Representation of an isy994mrf cover program."""

    def __init__(self, name: str, node: object, actions: object) -> None:
        """Initialize the isy994mrf cover program."""
        super().__init__(node)
        self._name = name
    ...
    @property
    def state(self) -> str:
        """Get the state of the isy994mrf cover program."""
        return STATE_CLOSED if bool(self.value) else STATE_OPEN

    def open_cover(self, **kwargs) -> None:
        """Send the open cover command to the isy994mrf cover program."""
        if not self._actions.runThen():
            _LOGGER.error("Unable to open the cover")

    def close_cover(self, **kwargs) -> None:
        """Send the close cover command to the isy994mrf cover program."""
